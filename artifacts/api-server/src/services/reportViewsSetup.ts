...
export async function ensureReportViews(logger: Logger): Promise<void> {
  ...
  try {
    ...
    await client.query(`
      ...
      lay_cogs AS (
        SELECT l.merchant_id, (COALESCE(l.completed_at, l.updated_at))::date AS sale_date,
          SUM((item->>'quantity')::numeric * COALESCE(p.cost_price::numeric, 0)) AS total_cogs
        FROM laybys l
    ...
    // ── Payment legs (per-method amounts) ──────────────────────────────────
    // Split payments record one leg per method, so revenue is attributed to the
    // actual method tendered rather than lumped under a single "settling" method.
    // These two views are the single source of truth for per-method reporting
    // (z-report, daily close, dashboard payment totals, the breakdown view).

    // Paid invoices: each payment event carrying an explicit amount is one leg.
    // Legacy payments recorded before per-leg amounts existed contribute a single
    // "remainder" leg (the unallocated amount_paid) to their last payment method.
    await client.query(`
    ...
    // Completed laybys: every recorded layby_payments row is a leg. A remainder
    // leg covers any gap (e.g. force-completed laybys), attributed to the last
    // recorded payment method, so the legs always sum to the layby total.
    await client.query(`
    ...
    await client.query(`
      ...
      WITH lines AS (
        ...
        UNION ALL
        -- Completed-layby line items (revenue = quantity × price; no cost snapshot)
        SELECT
          l.merchant_id,
          ...
          (item->>'quantity')::numeric * COALESCE(p.cost_price::numeric, 0) AS cogs
        FROM laybys l
        CROSS JOIN LATERAL jsonb_array_elements(CASE WHEN jsonb_typeof(l.items) = 'array' THEN l.items ELSE '[]'::jsonb END) AS item
        LEFT JOIN products p ON p.id = NULLIF(item->>'productId', '')::int AND p.merchant_id = l.merchant_id
