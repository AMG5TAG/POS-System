...
router.put("/pos-settings", requireAuth, async (req, res): Promise<void> => {
  ...
  const {
    ...
    gridShowStockBadges,
    gridCartPosition,
    quickViewShowSupplier,
    forceStaffLogin,
    promptCloseAllRegisters,
    staffLoginMessage,
    activeRegisterId,
    hardwareConfig,
    enabledShortcuts,
  ...
  const patch = {
    ...
    ...(quickViewShowSupplier !== undefined && { quickViewShowSupplier }),
    ...(forceStaffLogin !== undefined && { forceStaffLogin }),
    ...(promptCloseAllRegisters !== undefined && { promptCloseAllRegisters }),
