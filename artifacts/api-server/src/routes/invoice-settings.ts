...
const DEFAULT_CONFIG = {
  ...
  surchargePercent: 0,
  surchargeAfterReminders: 3,
  debtCollectionEnabled: false,
  debtCollectionThreshold: 3,
  // ── Sending ──
...
function formatSettings(row: typeof invoiceSettingsTable.$inferSelect | undefined): InvoiceConfig {
  ...
  return {
    ...
    surchargeEnabled: asBool(c.surchargeEnabled, DEFAULT_CONFIG.surchargeEnabled),
    surchargePercent: clampNum(c.surchargePercent, DEFAULT_CONFIG.surchargePercent),
    surchargeAfterReminders: clampInt(c.surchargeAfterReminders, DEFAULT_CONFIG.surchargeAfterReminders),
    debtCollectionEnabled: asBool(c.debtCollectionEnabled, DEFAULT_CONFIG.debtCollectionEnabled),
    debtCollectionThreshold: Math.max(1, clampInt(c.debtCollectionThreshold, DEFAULT_CONFIG.debtCollectionThreshold)),
