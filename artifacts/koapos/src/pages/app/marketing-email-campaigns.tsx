...
import {
  ...
  Link2, AlignLeft, AlignCenter, Undo2, Redo2, Eye,
  Clock, Trash2, Plus, Copy, FileText, Code, RefreshCw, Megaphone,
} from "lucide-react";
...
function apiToLocal(c: ApiCampaign): EmailCampaign {
  return {
    id: String(c.id ?? ""),
    name: String(c.name ?? ""),
    ...
    body: String(c.body ?? ""),
    ctaEnabled: c.ctaEnabled === "true" || c.ctaEnabled === true,
    ctaLabel: String(c.ctaLabel ?? "Shop Now"),
    ctaUrl: String(c.ctaUrl ?? ""),
...
const QUICK_TEMPLATES: QuickTemplate[] = [
  { id: "welcome",  label: "Welcome New Customer",      subject: "Welcome to {{business_name}}! 🎉",              body: `<p>Hi {{first_name}},</p><p><br></p><p>Welcome to <strong>{{business_name}}</strong>! We're so glad you've chosen us.</p><p><br></p><p>We can't wait to see you again.<br><strong>{{business_name}}</strong></p>` },
  { id: "promo",    label: "Promotional Offer",          subject: "🔥 Exclusive deal just for you, {{first_name}}", body: `<p>Hi {{first_name}},</p><p><br></p><p>We have an <strong>exclusive offer</strong> just for you! For a limited time only — don't miss out.</p><p><br></p><p>Thanks for being a valued customer.<br><strong>{{business_name}}</strong></p>` },
  { id: "thankyou", label: "Thank You",                  subject: "Thank you for shopping with us, {{first_name}}!",body: `<p>Hi {{first_name}},</p><p><br></p><p>Thank you so much for your recent purchase at <strong>{{business_name}}</strong>. Your support means the world to us!</p><p><br></p><p>See you next time!<br><strong>{{business_name}}</strong></p>` },
  { id: "loyalty",  label: "Loyalty Points Update",      subject: "Your loyalty balance: {{loyalty_points}} points 🌟", body: `<p>Hi {{first_name}},</p><p><br></p><p>You currently have <strong>{{loyalty_points}} loyalty points</strong> with us. They never expire!</p><p><br></p><p>Thanks for your continued support!<br><strong>{{business_name}}</strong></p>` },
  { id: "winback",  label: "Win-Back Campaign",          subject: "We miss you, {{first_name}}! 💛",               body: `<p>Hi {{first_name}},</p><p><br></p><p>It's been a while since we've seen you — <strong>we miss you!</strong> Come back and visit us soon.</p><p><br></p><p>Until then,<br><strong>{{business_name}}</strong></p>` },
  { id: "custom",   label: "Custom Message",             subject: "",                                              body: `<p>Hi {{first_name}},</p><p><br></p><p>Write your message here…</p><p><br></p><p>Regards,<br><strong>{{business_name}}</strong></p>` },
];
...
function RichToolbar({ editorRef }: { editorRef: React.RefObject<HTMLDivElement | null> }) {
  ...
  return (
    <div className="flex items-center gap-0.5 border-b px-2 py-1.5 bg-muted/30 flex-wrap">
      ...
      <button type="button" title="Insert link" onMouseDown={(e) => { e.preventDefault(); insertLink(); }} className="p-1.5 rounded hover:bg-muted transition-colors"><Link2 className="w-3.5 h-3.5" /></button>
      <button type="button" title="Undo" onMouseDown={(e) => { e.preventDefault(); exec("undo"); }} className="p-1.5 rounded hover:bg-muted transition-colors"><Undo2 className="w-3.5 h-3.5" /></button>
      <button type="button" title="Redo" onMouseDown={(e) => { e.preventDefault(); exec("redo"); }} className="p-1.5 rounded hover:bg-muted transition-colors"><Redo2 className="w-3.5 h-3.5" /></button>
    </div>
  );
}
...
function PreviewDialog({ open, onClose, subject, body, ctaEnabled, ctaLabel }: { open: boolean; onClose: () => void; subject: string; body: string; ctaEnabled: boolean; ctaLabel: string }) {
  const preview = body
    .replace(/\{\{first_name\}\}/g, "Sarah").replace(/\{\{last_name\}\}/g, "Johnson")
    .replace(/\{\{business_name\}\}/g, "Your Business").replace(/\{\{loyalty_points\}\}/g, "240")
    .replace(/\{\{unsubscribe_link\}\}/g, "#");
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-xl">
        <DialogHeader><DialogTitle className="flex items-center gap-2"><Eye className="w-4 h-4" /> Email Preview</DialogTitle></DialogHeader>
...
function CampaignRow({ campaign, onDelete }: { campaign: EmailCampaign; onDelete: (id: string) => void }) {
  const statusColor = {
  ...
  return (
    <div className="flex items-center gap-4 rounded-xl border bg-card px-4 py-3">
      <div className="rounded-lg bg-primary/10 p-2 shrink-0"><Mail className="w-4 h-4 text-primary" /></div>
      ...
      {campaign.status === "sent" && (
        <div className="text-right shrink-0 hidden sm:block">
          ...
          {campaign.bounces > 0 && <p className="text-xs text-red-500">{campaign.bounces} bounced</p>}
        </div>
      )}
  ...
  const saveDraft = () => {
    ...
    createCampaign.mutate({
      data: {
        ...
        name: draft.subject || "Draft",
        audience: draft.audience,
        audienceLabel: label,
        subject: draft.subject,
        body: draft.body,
        ctaEnabled: String(draft.ctaEnabled),
    ...
    <AppLayout>
      <div className="w-full px-4 lg:px-6 py-6 space-y-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2"><Send className="w-6 h-6 text-primary" /> Campaigns</h1>
            <p className="text-muted-foreground text-sm mt-1">Compose and send targeted emails to your customer base.</p>
          </div>
          <Link href="/marketing">
          ...
          <div className="space-y-4">
            <Card className="rounded-2xl">
              <CardHeader className="pb-2 pt-4 px-4">
                <CardTitle className="text-sm flex items-center gap-2"><FileText className="w-4 h-4 text-primary" /> Email Templates</CardTitle>
              </CardHeader>
            ...
            <Card className="rounded-2xl">
              <CardContent className="p-5 space-y-5">
                  ...
                  {draft.audience === "specific" && (
                    <div className="pt-1">
                      <Select value={draft.customerId ? String(draft.customerId) : ""} onValueChange={(v) => setField("customerId", v ? parseInt(v) : null)}>
                        ...
                        <SelectContent>
                          {allCustomers.filter((c) => c.email).sort((a, b) => `${a.firstName ?? ""}`.localeCompare(`${b.firstName ?? ""}`)).map((c) => (
                ...
                <div className="space-y-1.5">
                  ...
                  <Input value={draft.subject} onChange={(e) => setField("subject", e.target.value)} placeholder="e.g. 🎉 A special offer just for you, {{first_name}}" />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="border rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-ring">
                    ...
                    <div ref={editorRef} contentEditable suppressContentEditableWarning onInput={handleBodyInput}
                ...
                <div className="rounded-xl border p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <Label className="flex items-center gap-2 cursor-pointer"><Link2 className="w-3.5 h-3.5 text-muted-foreground" /> CTA Button (optional)</Label>
                    <Switch checked={draft.ctaEnabled} onCheckedChange={(v) => setField("ctaEnabled", v)} />
                  </div>
                  {draft.ctaEnabled && (
                    <div className="grid grid-cols-2 gap-3 pt-1">
                      <div className="space-y-1"><Label className="text-xs">Button Label</Label><Input value={draft.ctaLabel} onChange={(e) => setField("ctaLabel", e.target.value)} placeholder="Shop Now" /></div>
                      <div className="space-y-1"><Label className="text-xs">Link URL</Label><Input value={draft.ctaUrl} onChange={(e) => setField("ctaUrl", e.target.value)} placeholder="https://yourstore.com" /></div>
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2 pt-1">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => setPreviewOpen(true)} title="Preview"><Eye className="w-4 h-4" /></Button>
      ...
      <PreviewDialog open={previewOpen} onClose={() => setPreviewOpen(false)} subject={draft.subject} body={draft.body} ctaEnabled={draft.ctaEnabled} ctaLabel={draft.ctaLabel} />
    </AppLayout>
  );
}
