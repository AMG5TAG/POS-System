...
import {
  Globe, Plus, Trash2, ExternalLink, Copy, ChevronUp, ChevronDown,
  Smartphone, Image, Link2, Palette, AlignLeft, Eye, ArrowLeft,
    ...
    >
      ...
      {page.profileImage ? (
        <img
          ...
          alt={page.title}
          className="w-20 h-20 rounded-full object-cover mb-4 ring-4 ring-white/30"
        />
      ) : (
        <div className="w-20 h-20 rounded-full bg-white/20 flex items-center justify-center mb-4 text-3xl">
          🏪
        </div>
      )}
      ...
      {page.subtitle && (
        <p className="text-sm mt-1 text-center opacity-80" style={{ color: page.textColor }}>
          {page.subtitle}
        </p>
      )}
      {page.bio && (
        <p className="text-xs mt-2 text-center opacity-70 max-w-xs leading-relaxed" style={{ color: page.textColor }}>
          {page.bio}
        </p>
      )}
...
function LinkRow({
  link, onUpdate, onDelete, onMove, isFirst, isLast,
}: {
  link: LandingPageLink;
  onUpdate: (patch: Partial<LandingPageLink>) => void;
  onDelete: () => void;
    ...
    <div className={cn("border rounded-lg overflow-hidden", !link.enabled && "opacity-50")}>
      <div className="flex items-center gap-2 px-3 py-2 bg-muted/30">
        ...
        <Input
          ...
          className="w-10 h-7 text-sm text-center px-1 font-mono bg-background"
          placeholder="🔗"
          maxLength={2}
        />
          ...
          <>
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <div
                  className="w-16 h-16 rounded-full bg-muted border-2 border-dashed border-border overflow-hidden flex items-center justify-center cursor-pointer hover:border-primary/50 transition-colors"
        ...
        {tab === "style" && (
          <>
            ...
            <div>
              ...
              {page.bgType === "color" && (
                <ColourPicker value={page.bgColor} onChange={(v) => set("bgColor", v)} />
              )}
              {page.bgType === "gradient" && (
                <div className="space-y-2">
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <ColourPicker value={page.bgFrom} onChange={(v) => set("bgFrom", v)} />
                    </div>
                    <div>
                      <Label className="text-xs mb-1 block">To</Label>
                      <ColourPicker value={page.bgTo} onChange={(v) => set("bgTo", v)} />
                    </div>
                  </div>
                  <div className="space-y-1">
                    ...
                    <div className="grid grid-cols-3 gap-1">
                      {["to bottom", "to right", "135deg"].map((d) => (
                        <button key={d} onClick={() => set("bgDir", d)}
                          className={cn("py-1 text-xs rounded border transition-colors",
                            page.bgDir === d ? "bg-primary/10 border-primary text-primary" : "border-border text-muted-foreground hover:bg-muted/50"
                          )}>
              ...
              {page.bgType === "image" && (
                <div className="space-y-2">
                  ...
                  <Button type="button" variant="outline" size="sm" className="w-full gap-1.5"
                    onClick={() => handleImageUpload(bgImageRef, "bgImage")}>
                    <Image className="w-3.5 h-3.5" />
                    {page.bgImage ? "Change background image" : "Upload background image"}
                  </Button>
                  {page.bgImage && (
            ...
            <div className="space-y-1">
              ...
              <ColourPicker value={page.textColor} onChange={(v) => set("textColor", v)} />
            </div>

            {/* Font */}
            <div className="space-y-1">
              <Label className="text-xs">Font</Label>
              <FontPicker value={page.font} onChange={(v) => set("font", v)} />
            </div>
              ...
              <div className="flex gap-2">
                {(["pill", "rounded", "square"] as const).map((s) => (
                  <button key={s} onClick={() => set("btnStyle", s)}
                    className={cn("flex-1 py-1.5 text-xs font-medium border transition-colors capitalize",
                      { pill: "rounded-full", rounded: "rounded-lg", square: "rounded-sm" }[s],
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label className="text-xs">Button fill</Label>
                <ColourPicker value={page.btnBg} onChange={(v) => set("btnBg", v)} />
              </div>
              <div className="space-y-1">
                <Label className="text-xs">Button text</Label>
                <ColourPicker value={page.btnText} onChange={(v) => set("btnText", v)} />
              </div>
            </div>
            {page.btnVariant === "outline" && (
              <div className="space-y-1">
                <Label className="text-xs">Button border</Label>
                <ColourPicker value={page.btnBorder} onChange={(v) => set("btnBorder", v)} />
              </div>
            )}
          </>
        )}
          ...
          <>
            ...
            {page.links.length === 0 && (
              <p className="text-xs text-muted-foreground text-center py-2">
                No links yet. Add links that will appear as buttons on your landing page.
              </p>
            )}
          </>
        )}
...
function PagesListView({
  pages, onSelect, onCreate, onDelete,
}: {
  pages: LandingPage[];
            ...
            <Card key={page.id} className="overflow-hidden cursor-pointer hover:border-primary/50 hover:shadow-md transition-all group">
              ...
              >
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 p-4">
                  {page.profileImage
                    ? <img src={page.profileImage} alt="" className="w-12 h-12 rounded-full object-cover ring-2 ring-white/40" />
                    : <div className="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center text-xl">🏪</div>
                  }
                  <p className="text-sm font-bold truncate max-w-full" style={{ color: page.textColor, fontFamily: page.font ? `"${page.font}", sans-serif` : "inherit" }}>
                    {page.title}
                  </p>
                  <p className="text-xs opacity-70 truncate max-w-full text-center" style={{ color: page.textColor }}>
                    {page.subtitle}
                  </p>
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <Button variant="outline" size="icon" className="h-7 w-7"
                      onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(getPageUrl(page.slug)); toast.success("Link copied"); }}>
                      <Copy className="w-3 h-3" />
                    </Button>
                    <Button variant="outline" size="icon" className="h-7 w-7"
                      onClick={(e) => { e.stopPropagation(); window.open(getPageUrl(page.slug), "_blank"); }}>
                      <ExternalLink className="w-3 h-3" />
                    </Button>
                    <Button variant="outline" size="icon" className="h-7 w-7" onClick={() => onSelect(page.id)}>
...
export default function MarketingLandingPagesPage() {
  ...
  const updatePages = (next: LandingPage[]) => {
    setPages(next);
    savePages(next);
  };
  ...
  const handleChange = useCallback((patch: Partial<LandingPage>) => {
    ...
    const now = new Date().toISOString();
    const next = pages.map((p) => p.id === selectedId ? { ...p, ...patch, updatedAt: now } : p);
    updatePages(next);
    setSaveState("saved");
  ...
  if (!selected) {
    return (
      <AppLayout>
        <PagesListView pages={pages} onSelect={setSelectedId} onCreate={createPage} onDelete={deletePage} />
      </AppLayout>
    );
  }
    ...
    <AppLayout>
      <div className="h-[calc(100dvh-3.5rem)] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-4 py-2 border-b bg-background shrink-0 flex-wrap gap-y-2">
          <Button variant="ghost" size="sm" className="gap-1.5" onClick={() => setSelectedId(null)}>
