...
export default function MarketingReferralsPage() {
  ...
  const { data, isLoading } = useListCustomers({ limit: 500, offset: 0 });
  ...
  const referredCustomers = useMemo(() => {
    return customers.filter((c) => c.referredBy && codeToName.has(c.referredBy));
  ...
  const statsByCustomer = useMemo(() => {
    ...
    for (const c of enrolled) {
      stats.set(c.id, { referrals: 0, qualified: 0, pending: 0, points: 0 });
    }
    for (const r of referredCustomers) {
      const code = r.referredBy!;
      // Find the referrer by matching code (there may be duplicates, take first)
      const referrer = enrolled.find((c) => c.referralCode === code);
      if (!referrer) continue;
      const s = stats.get(referrer.id)!;
      s.referrals += 1;
      const days = daysSince(r.createdAt);
      const isQualified = ((r.totalSpent ?? 0) >= settings.minSpend) && ((r.visitCount ?? 0) >= settings.minVisits) && (days <= settings.qualifyDays);
      if (isQualified) {
        s.qualified += 1;
        s.points += settings.referrerRewardAmount;
      } else {
        s.pending += 1;
      }
    }
    return stats;
  }, [enrolled, referredCustomers, settings]);
            ...
            <CardContent className="space-y-3">
              {referredCustomers.length === 0 ? (
                ...
                referredCustomers.map((r) => {
                  ...
                  const visitPct = Math.min(100, ((r.visitCount ?? 0) / settings.minVisits) * 100);
                  const days = daysSince(r.createdAt);
