...
import { Plus, Bookmark, Pencil, Trash2, Search, Globe } from "lucide-react";
import { toast } from "sonner";

type Brand = { id: number; name: string; description: string | null; website: string | null; createdAt: string };
...
export default function ProductsBrandsPage() {
  ...
  const openCreate = () => { setEditing(null); setForm({ name: "", description: "", website: "" }); setDialogOpen(true); };
  const openEdit = (b: Brand) => { setEditing(b); setForm({ name: b.name, description: b.description ?? "", website: b.website ?? "" }); setDialogOpen(true); };
