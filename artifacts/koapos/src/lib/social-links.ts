...
/**
 * Single-path brand glyphs (viewBox "0 0 24 24", filled with currentColor)
 * for each supported platform. Inlined here — no external requests — so the
 * icons survive printing and PDF export. Sourced from Simple Icons (CC0),
 * with classic LinkedIn/Twitter marks added for the two platforms Simple
...
/**
 * Returns the raw SVG path data (viewBox "0 0 24 24") for a platform key,
 * or null when no icon is known. Use with `<path d={...} fill="currentColor" />`.
 */
export function getSocialIconPath(key: string): string | null {
  return SOCIAL_ICON_PATHS[key.toLowerCase()] ?? null;
}
...
/**
 * Converts a raw social-links map to an array of display entries.
 * Only entries with a non-empty value are included. Each entry carries the
 * friendly label, the short handle, the platform key, and the inline SVG icon
 * path (null when no icon is available for that platform).
 */
export function formatSocialEntries(
  links: Record<string, string> | undefined | null,
): Array<{ label: string; handle: string; key: string; iconPath: string | null }> {
  if (!links) return [];
