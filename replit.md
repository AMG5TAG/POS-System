...
- Gift-card payments are settled server-side and atomically: the client sends `giftCardPayment {cardId, amount}` on `POST /transactions` (and `POST /invoices/:id/payment`), and the server locks the card row (`FOR UPDATE`), validates, debits it, and writes the redemption ledger entry inside the SAME db transaction that records the sale/payment — so the card can never be charged without the sale landing (or vice-versa). There is no client-side debit + compensation.
- Idempotency: both endpoints accept an `idempotencyKey`. `transactions` has a unique index on `(merchantId, idempotencyKey)` and returns the original transaction on a retry; invoice payments dedupe via a matching key recorded in the invoice's events. The POS generates one key per checkout attempt (reused across manual retries, reset on success/clear-cart).
